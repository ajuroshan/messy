...
class Messcut(models.Model):
    ...
    def get_date_range(self):
        ...
        return [
            self.start_date + datetime.timedelta(days=i) for i in range(delta.days + 1)
        ]
...
class Messmenu(models.Model):
    ...
    day = models.CharField(
        max_length=9, choices=DAYS_OF_WEEK, unique=True, blank=True, null=True
    )
...
class MessAttendance(models.Model):
    ...
    student = models.ForeignKey(
        "application.Application", on_delete=models.CASCADE, blank=True, null=True
    )
    hostel = models.ForeignKey(
        "application.Hostel", on_delete=models.CASCADE, blank=True, null=True
    )
...
class Messsettings(models.Model):
    ...
    mess_secretary_upi_qr = models.ImageField(
        upload_to="upi_qr_codes", blank=True, null=True
    )
    sagar_post_metric_upi_qr = models.ImageField(
        upload_to="upi_qr_codes", blank=True, null=True
    )
