...
class ApplicationAdmin(admin.ModelAdmin):
    ...
    def accept_application(self, request, queryset):
        ...
        self.message_user(
            request,
            f"{updated_count} applications were successfully marked as verified.",
        )
    ...
    def make_mess_assistant(self, request, queryset):
        ...
        mess_assistants_group, created = Group.objects.get_or_create(
            name="mess_assistants"
        )
        ...
        for application in queryset:
            ...
            if not user.groups.filter(name="mess_assistants").exists():
                ...
                self.message_user(
                    request, f"User {user.username} added to mess_assistants group."
                )
            else:
                self.message_user(
                    request, f"User {user.username} is already a mess_assistant."
                )
    ...
    def dismiss_mess_assistant(self, request, queryset):
        ...
        for application in queryset:
            ...
            if user.groups.filter(name="mess_assistants").exists():
                ...
                self.message_user(
                    request, f"User {user.username} removed from mess_assistants group."
                )
            else:
                self.message_user(
                    request, f"User {user.username} is not a mess_assistant."
                )
